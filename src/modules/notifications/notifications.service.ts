...
import { NotificationType, NotifStatus, VehicleRecord } from '@prisma/client';

const MSG_PREFIX = 'A2insurance care: ';
...
export class NotificationsService {
  ...
  async getLogs(limit = 100) {
    return this.prisma.notificationLog.findMany({
      ...
      include: {
        ...
        healthInsurance: {
          select: { policyNumber: true, policyHolderName: true },
        },
      },
    });
  }
  ...
  async getLogStats() {
    const [sent, failed, pending] = await Promise.all([
      this.prisma.notificationLog.count({
        where: { status: NotifStatus.SENT },
      }),
      this.prisma.notificationLog.count({
        where: { status: NotifStatus.FAILED },
      }),
      this.prisma.notificationLog.count({
  ...
  private buildExpiryMessage(
    ...
    if (daysLeft <= 0) {
      return isTamil
        ? `அன்பான வாடிக்கையாளரே உங்கள் வாகனம்\n*${vehicleNumber}* இன்சூரன்ஸ் பாலிசி காலாவதி தேதி *${dateStr}*${footer}`
        : `Dear Valuable Customer your Vehicle\n*${vehicleNumber}* Insurance policy expiry on *${dateStr}*${footer}`;
    }
  ...
  async sendExpiryAlert(
    ...
    if (await this.hasSentRecently(vehicle.id, type)) {
      this.logger.log(
        `Skipping ${type} for ${vehicle.vehicleNumber} — already sent recently`,
      );
      return;
    }
    ...
    this.logger.log(
      `${type} → ${vehicle.vehicleNumber} (${vehicle.cellNumber}): ${result.success ? 'SENT' : 'FAILED'}`,
    );

    const expStr = vehicle.policyExpiryDate.toLocaleDateString('en-IN', {
      day: '2-digit',
      month: 'short',
      year: 'numeric',
    });
  ...
  async sendRenewalAlert(vehicle: {
    ...
    const dateStr = vehicle.policyExpiryDate.toLocaleDateString('en-IN', {
      day: '2-digit',
      month: 'short',
      year: 'numeric',
    });
  ...
  async sendHealthRenewalAlert(
    ...
    const message =
      MSG_PREFIX +
      this.buildHealthRenewalMessage(
        health.policyHolderName,
        health.policyNumber,
        health.renewalDate,
        daysLeft,
        settings.language,
    ...
    this.logger.log(
      `Health ${type} → ${health.policyNumber} (${health.mobileNumber}): ${result.success ? 'SENT' : 'FAILED'}`,
    );
  }
  ...
  async processScheduledHealthAlerts() {
    ...
    const [renewal30, renewal15, renewal7, overdue] = await Promise.all([
      this.prisma.healthInsurance.findMany({
        where: {
          renewalDate: buildRange(settings.firstAlertDays),
          policyStatus: { not: 'CANCELLED' },
        },
      }),
      this.prisma.healthInsurance.findMany({
        where: {
          renewalDate: buildRange(settings.secondAlertDays),
          policyStatus: { not: 'CANCELLED' },
        },
      }),
      this.prisma.healthInsurance.findMany({
        where: {
          renewalDate: buildRange(settings.finalAlertDays),
          policyStatus: { not: 'CANCELLED' },
        },
      }),
      this.prisma.healthInsurance.findMany({
        where: {
          renewalDate: { gte: new Date(now.getTime() - 7 * MSperDay), lt: now },
          policyStatus: { not: 'CANCELLED' },
        },
      }),
    ]);
    ...
    const jobs: HealthJob[] = [
      ...renewal30.map((r) => ({
        record: r,
        type: NotificationType.EXPIRY_30,
        days: settings.firstAlertDays,
      })),
      ...renewal15.map((r) => ({
        record: r,
        type: NotificationType.EXPIRY_15,
        days: settings.secondAlertDays,
      })),
      ...renewal7.map((r) => ({
        record: r,
        type: NotificationType.EXPIRY_7,
        days: settings.finalAlertDays,
      })),
      ...overdue.map((r) => ({
        record: r,
    ...
    for (const job of jobs) {
      try {
        ...
        this.logger.error(
          `Health alert failed for ${job.record.policyNumber}: ${this.errMsg(err)}`,
        );
      }
    }
  }

  private buildFireRenewalMessage(
    insuredName: string,
    policyNumber: string,
    renewalDate: Date,
    daysLeft: number,
    ...
    if (daysLeft <= 0) {
      return isTamil
        ? `அன்புள்ள ${insuredName},\n\n⚠️ உங்கள் தீ காப்பீடு (*${policyNumber}*) ${dateStr} அன்று *புதுப்பிக்க வேண்டிய தேதி*.\n\nதாமதிக்காமல் புதுப்பிக்கவும்.${footer}`
        : `Dear ${insuredName},\n\n⚠️ Your fire insurance policy (*${policyNumber}*) renewal was due on ${dateStr}.\n\nPlease renew immediately to avoid a coverage gap.${footer}`;
    }

    if (daysLeft <= 7) {
      return isTamil
        ? `அன்புள்ள ${insuredName},\n\n🚨 *அவசரம்!* உங்கள் தீ காப்பீடு (*${policyNumber}*) வெறும் *${daysLeft} நாட்களில்* (${dateStr}) புதுப்பிக்க வேண்டும்.\n\nதாமதிக்காமல் இப்போதே தொடர்பு கொள்ளவும்.${footer}`
        : `Dear ${insuredName},\n\n🚨 *URGENT!* Your fire insurance policy (*${policyNumber}*) renewal is due in just *${daysLeft} days* (${dateStr}).\n\nPlease contact us immediately to renew your coverage.${footer}`;
    }

    if (daysLeft <= 15) {
      return isTamil
        ? `அன்புள்ள ${insuredName},\n\n🔔 நினைவூட்டல்: உங்கள் தீ காப்பீடு (*${policyNumber}*) *${daysLeft} நாட்களில்* (${dateStr}) புதுப்பிக்க வேண்டும்.\n\nசீக்கிரமே புதுப்பிக்கவும்.${footer}`
        : `Dear ${insuredName},\n\n🔔 Reminder: Your fire insurance policy (*${policyNumber}*) renewal is due in *${daysLeft} days* (${dateStr}).\n\nPlease renew soon to ensure continuous coverage.${footer}`;
    }

    return isTamil
      ? `அன்புள்ள ${insuredName},\n\n📋 முன்னறிவிப்பு: உங்கள் தீ காப்பீடு (*${policyNumber}*) *${daysLeft} நாட்களில்* (${dateStr}) புதுப்பிக்க வேண்டும்.\n\nசமயத்தில் புதுப்பிக்க திட்டமிடவும்.${footer}`
      : `Dear ${insuredName},\n\n📋 Advance Notice: Your fire insurance policy (*${policyNumber}*) renewal is due in *${daysLeft} days* (${dateStr}).\n\nPlease plan for renewal in advance.${footer}`;
  }

  async sendFireRenewalAlert(
    fire: {
      id: number;
      policyNumber: string;
      insuredName: string;
    ...
    const settings = await this.getSettings();
    if (!settings.enableWhatsApp) {
      this.logger.warn('WhatsApp disabled — skipping fire alert');
      return;
    }
    if (!this.whatsapp.isReady()) {
      this.logger.warn('WhatsApp not connected — skipping fire alert');
      return;
    }

    const message =
      MSG_PREFIX +
      this.buildFireRenewalMessage(
        fire.insuredName,
        fire.policyNumber,
        fire.renewalDate,
        daysLeft,
    ...
    const log = await this.prisma.notificationLog.create({
      data: {
        fireInsuranceId: fire.id,
        mobileNumber: fire.mobileNumber,
        notificationType: type,
        message,
        status: NotifStatus.PENDING,
      },
    ...
    const result = await this.whatsapp.send(fire.mobileNumber, message);
    ...
    this.logger.log(
      `Fire ${type} → ${fire.policyNumber} (${fire.mobileNumber}): ${result.success ? 'SENT' : 'FAILED'}`,
    );
  }

  async processScheduledFireAlerts() {
    const settings = await this.getSettings();
    if (!settings.enableWhatsApp) return;
    ...
    const [renewal30, renewal15, renewal7, overdue] = await Promise.all([
      this.prisma.fireInsurance.findMany({
        where: {
          renewalDate: buildRange(settings.firstAlertDays),
          policyStatus: { not: 'CANCELLED' },
        },
    ...
    this.logger.log(`Found ${jobs.length} fire insurance alerts to process`);

    for (const job of jobs) {
      try {
        await this.sendFireRenewalAlert(job.record, job.type, job.days);
      } catch (err: unknown) {
        this.logger.error(
          `Fire alert failed for ${job.record.policyNumber}: ${this.errMsg(err)}`,
        );
      }
    }
  }

  private buildLabourRenewalMessage(
    insuredName: string,
    policyNumber: string,
    renewalDate: Date,
    daysLeft: number,
    ...
    if (daysLeft <= 0) {
      return isTamil
        ? `அன்புள்ள ${insuredName},\n\n⚠️ உங்கள் தொழிலாளர் காப்பீடு (*${policyNumber}*) ${dateStr} அன்று *புதுப்பிக்க வேண்டிய தேதி*.\n\nதாமதிக்காமல் புதுப்பிக்கவும்.${footer}`
        : `Dear ${insuredName},\n\n⚠️ Your labour insurance policy (*${policyNumber}*) renewal was due on ${dateStr}.\n\nPlease renew immediately to avoid a coverage gap.${footer}`;
    }

    if (daysLeft <= 7) {
      return isTamil
        ? `அன்புள்ள ${insuredName},\n\n🚨 *அவசரம்!* உங்கள் தொழிலாளர் காப்பீடு (*${policyNumber}*) வெறும் *${daysLeft} நாட்களில்* (${dateStr}) புதுப்பிக்க வேண்டும்.\n\nதாமதிக்காமல் இப்போதே தொடர்பு கொள்ளவும்.${footer}`
        : `Dear ${insuredName},\n\n🚨 *URGENT!* Your labour insurance policy (*${policyNumber}*) renewal is due in just *${daysLeft} days* (${dateStr}).\n\nPlease contact us immediately to renew your coverage.${footer}`;
    }

    if (daysLeft <= 15) {
      return isTamil
        ? `அன்புள்ள ${insuredName},\n\n🔔 நினைவூட்டல்: உங்கள் தொழிலாளர் காப்பீடு (*${policyNumber}*) *${daysLeft} நாட்களில்* (${dateStr}) புதுப்பிக்க வேண்டும்.\n\nசீக்கிரமே புதுப்பிக்கவும்.${footer}`
        : `Dear ${insuredName},\n\n🔔 Reminder: Your labour insurance policy (*${policyNumber}*) renewal is due in *${daysLeft} days* (${dateStr}).\n\nPlease renew soon to ensure continuous coverage.${footer}`;
    }

    return isTamil
      ? `அன்புள்ள ${insuredName},\n\n📋 முன்னறிவிப்பு: உங்கள் தொழிலாளர் காப்பீடு (*${policyNumber}*) *${daysLeft} நாட்களில்* (${dateStr}) புதுப்பிக்க வேண்டும்.\n\nசமயத்தில் புதுப்பிக்க திட்டமிடவும்.${footer}`
      : `Dear ${insuredName},\n\n📋 Advance Notice: Your labour insurance policy (*${policyNumber}*) renewal is due in *${daysLeft} days* (${dateStr}).\n\nPlease plan for renewal in advance.${footer}`;
  }

  async sendLabourRenewalAlert(
    labour: {
      id: number;
      policyNumber: string;
      insuredName: string;
    ...
    const settings = await this.getSettings();
    if (!settings.enableWhatsApp) {
      this.logger.warn('WhatsApp disabled — skipping labour alert');
      return;
    }
    if (!this.whatsapp.isReady()) {
      this.logger.warn('WhatsApp not connected — skipping labour alert');
      return;
    }

    const message =
      MSG_PREFIX +
      this.buildLabourRenewalMessage(
        labour.insuredName,
        labour.policyNumber,
        labour.renewalDate,
        daysLeft,
    ...
    const result = await this.whatsapp.send(labour.mobileNumber, message);
    ...
    this.logger.log(
      `Labour ${type} → ${labour.policyNumber} (${labour.mobileNumber}): ${result.success ? 'SENT' : 'FAILED'}`,
    );
  }

  async processScheduledLabourAlerts() {
    const settings = await this.getSettings();
    if (!settings.enableWhatsApp) return;
    ...
    const [renewal30, renewal15, renewal7, overdue] = await Promise.all([
      this.prisma.labourInsurance.findMany({
        where: {
          renewalDate: buildRange(settings.firstAlertDays),
          policyStatus: { not: 'CANCELLED' },
        },
    ...
    this.logger.log(`Found ${jobs.length} labour insurance alerts to process`);

    for (const job of jobs) {
      try {
        await this.sendLabourRenewalAlert(job.record, job.type, job.days);
      } catch (err: unknown) {
        this.logger.error(
          `Labour alert failed for ${job.record.policyNumber}: ${this.errMsg(err)}`,
        );
      }
    }
  }
  ...
  async sendManual(dto: SendManualDto) {
    if (!this.whatsapp.isReady()) {
      return {
        success: false,
        message: 'WhatsApp not connected. Go to Settings to scan the QR code.',
      };
    }
    ...
    const log = await this.prisma.notificationLog.create({
      data: {
        ...
        healthInsuranceId: dto.healthInsuranceId ?? null,
        fireInsuranceId: dto.fireInsuranceId ?? null,
        labourInsuranceId: dto.labourInsuranceId ?? null,
    ...
    const result = await this.whatsapp.send(dto.mobileNumber, message);
    ...
    return {
      ...
      message: result.success ? 'Message sent' : result.response,
    };
  }
  ...
  // Pushed once per location (plus once for legacy records with no location
  // assigned) so an Admin User only ever sees their own location's policy
  // summary — Master/Super Admin are in every location's room and see all of them.
  async pushPolicySummary() {
    const locations = await this.prisma.location.findMany({
      select: { id: true },
    });
    const locationIds: Array<number | null> = [
    ...
    await Promise.all(
      locationIds.map((locationId) =>
        this.pushPolicySummaryForLocation(locationId),
      ),
    );
  }

  private async pushPolicySummaryForLocation(locationId: number | null) {
    const now = new Date();
    const MSperDay = 86_400_000;
    ...
    const [expiredList, todayList, week7, week15, week30] = await Promise.all([
      this.prisma.vehicleRecord.findMany({
        where: { locationId, policyExpiryDate: { lt: startToday } },
        select: {
          vehicleNumber: true,
          ownerName: true,
      ...
      this.prisma.vehicleRecord.findMany({
        where: {
          ...
          policyExpiryDate: {
            gte: startToday,
            lt: new Date(startToday.getTime() + MSperDay),
          },
        },
        select: {
          vehicleNumber: true,
      ...
      this.prisma.vehicleRecord.count({
        where: { locationId, policyExpiryDate: { gte: startToday, lte: end7 } },
      }),
      this.prisma.vehicleRecord.count({
        where: { locationId, policyExpiryDate: { gt: end7, lte: end15 } },
      }),
      this.prisma.vehicleRecord.count({
        where: { locationId, policyExpiryDate: { gt: end15, lte: end30 } },
      }),
    ]);
      ...
      this.gateway.push(
        {
          ...
          message: todayList
            .map((v) => `${v.vehicleNumber} · ${v.ownerName}`)
            .join('\n'),
          severity: 'error',
        },
        locationId,
      ...
      this.gateway.push(
        {
          ...
          message: expiredList
            .map(
              (v) =>
                `${v.vehicleNumber} · ${v.ownerName} · expired ${fmt(v.policyExpiryDate)}`,
            )
            .join('\n'),
          severity: 'warning',
        },
  ...
  async cleanupOldLogs(days = 2): Promise<number> {
    ...
    this.logger.log(
      `Log cleanup: deleted ${count} records older than ${days} days`,
    );
    return count;
  }
  ...
  async processScheduledAlerts() {
    ...
    const [expiry30, expiry15, expiry7, expired] = await Promise.all([
      this.prisma.vehicleRecord.findMany({
        where: { policyExpiryDate: buildRange(settings.firstAlertDays) },
      }),
      this.prisma.vehicleRecord.findMany({
        where: { policyExpiryDate: buildRange(settings.secondAlertDays) },
      }),
      this.prisma.vehicleRecord.findMany({
        where: { policyExpiryDate: buildRange(settings.finalAlertDays) },
      }),
      this.prisma.vehicleRecord.findMany({
        where: {
          policyExpiryDate: {
            gte: new Date(now.getTime() - 7 * MSperDay),
            lt: now,
          },
        },
      }),
    ...
    for (const job of jobs) {
      try {
        ...
        this.logger.error(
          `Alert failed for ${job.vehicle.vehicleNumber}: ${this.errMsg(err)}`,
        );
      }
    }
    ...
    // ── Fire Insurance renewal alerts ─────────────────────────────
    await this.processScheduledFireAlerts();

    // ── Labour Insurance renewal alerts ───────────────────────────
    await this.processScheduledLabourAlerts();
  ...
  async processTodayAlerts() {
    ...
    for (const v of vehicles) {
      try {
        ...
        this.logger.error(
          `Today alert failed for ${v.vehicleNumber}: ${this.errMsg(err)}`,
        );
      }
    }
  }
